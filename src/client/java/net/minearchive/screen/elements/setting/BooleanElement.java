...
public class BooleanElement extends AbstractElement<BooleanSetting> {
    ...
    public BooleanElement(BooleanSetting setting, float x, float y, float width, float height) {
        ...
        this.colorT = new ColorAnimation(new Color(setting.getValue() ? 0xFFFAC0FF : 0xFF323232), EnumEasing.SINE.getEasing());
        this.colorB = new ColorAnimation(new Color(setting.getValue() ? 0xFFB3A5FF : 0xFF323232), EnumEasing.SINE.getEasing());
    ...
    public void render(DrawContext context, double mouseX, double mouseY, float delta, float offset) {
        this.offset = offset;
        colorT.setAnimation(new Color(t.getValue() ? 0xFFFAC0FF : 0xFF323232), 150);
        colorB.setAnimation(new Color(t.getValue() ? 0xFFB3A5FF : 0xFF323232), 150);
